...
논문의 저자는 잔디가 바람과 일관되지 않은 모션을 발생시키는 것을 막기 위해 몇가지 추가적인 절차를 사용합니다. 
그러나 그중에는 위의 문제점들과 더욱 나쁜 시너지를 일으키게 되어 있는것도 있습니다.

논문의 저자는 다음과 같은 추가적인 방식을 사용하여 지면과 잔디 조인트의 토크를 계산합니다.

- 지면의 조인트는 유니버설 조인트로 제한. (지면의 노멀 축과 잔디의 로컬 Y 축으로만 회전하는 조인트) 
- 이 조인트가 받는 토크는 잔디의 끝부분에서 받는 힘으로 계산 됨.

![distorted_swing](./resources/distorted_swing.jpeg)

위 그림과 같이 바람의 힘이 가해져서 잔디가 Z축으로 회전해야하는 경우.
- 잔디에 가해지는 토크는 빨간 색 벡터가 가리키는 잔디의 끝부분 포인트에 발생하는 것으로 계산됨.
- 파란색 벡터가 가리키는 포인트에 가해지는 바람 포스는 무시됨.

토크는 조인트의 거리와 비례해서 늘어납니다. 위 잔디는 본래 받아야 할 토크 보다 훨씬 적은 토크를 받게 됩니다.

극단적인 경우로 만약 잔디의 끝부분이 베이스 부분과 맞닿게 된다면 바람 포스가 가해지는 포인트와 조인트의 거리는 0이 되며 잔디는 0 토크를 받게 됩니다.
...
다음은 참고 논문에서처럼 바람과 공기 마찰을 포인트 포스 취급하고, 각 관절에서  무시하여 만든 모션 영상입니다.  
단, 논문에서 다음과 사항들은 수정하였습니다.
- 계산 오류 수정
